  ...
  */
  ...
  int N_bins=bins.size()-1;
    ...
    case dg1zlZToCWWW:
      ...
      ret = (P[0]->GetBinContent(1,bin_with_x)
	     +P[0]->GetBinContent(2,bin_with_x)*v1*3.*g*g*MW*MW/2.
	     +P[0]->GetBinContent(5,bin_with_x)*v1*3.*g*g*MW*MW/2.*v1*3.*g*g*MW*MW/2.
	     )*SM_shape;      
      break;
    case notype:
      assert(NULL && "invalid limit type!");
  ...
  for(Int_t i=0 ; i<N_bins ; i++) {
    ...
    case dg1zlZToCWWW:
      ...
      ret += (P[0]->GetBinContent(1,i+1)
	     ...
	     +P[0]->GetBinContent(5,i+1)*v1*3.*g*g*MW*MW/2.*v1*3.*g*g*MW*MW/2.)*integral_basis[rName][i];
      break;
    default:
      assert(code==1 && "invalid limit type!");
      break;
